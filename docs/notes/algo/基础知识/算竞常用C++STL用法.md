...
### 2.2.4 vector -&gt; deque双端队列

#### 额外特性

特性和vector一样，区别是可以快速对头部进行插入和删除操作

```cpp
deque<int> dq;
dq.push_front(114514);
dq.pop_front();
```
...
- 元素是否出现过：元素大小[-10^18, 10^18] ，元素数量10^6 ，vis 数组无法实现，通过 set 可以完成。
...
在C++中，我们可以通过结构体来封装函数，算法竞赛中，我们一般在并查集中使用
...
对于二元组pair和三元组tupe，默认的比较逻辑都是从头到尾按顺序比较。

但在结构体中，我们没有这种默认规则，因此我们需要重载运算符



先补充一个知识点，在c++中，一般的常用算法中，只会用到"&lt;"这个运算符，"&gt;"和"="一般都不会用到
...
struct Node{
    ...
    bool operator < (const Node &other) const {
		int sum=x+y+z;
        int osum=other.x+other.y+other.z;
        return sum<osum
    }
}
```

对于对结构体进行封装的容器，使用operator进行重载后就不需要再自己重写一个函数/结构体来更改比较逻辑了

```cpp
vector<Node> v;
priority_queue<Node,vector<Node>> pq;
//这里就会根据结构体中的比较逻辑来进行比较了
```
